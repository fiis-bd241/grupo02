...
4. Eliminar Pregunta: Se Eliminirá una pregunta dependiendo del tipo de cuestionario.
    ...
    WHERE ID_Cuestionario IN (
        ...
        WHERE Cu.ID_Tipo_Cuestionario =<1>
    )
    AND ID_Pregunta = <4>;

4. Estado de envío: Se muestra el estado de envío del cuestionario.
    Select Estado_Envio from Cuestionario where Id_Tipo_Cuestionario=<2>;

5. Estado de aprobación: Se muestra el estado de aprobación del cuestionario.
	Select Estado_Aprobacion from Cuestionario where Id_Tipo_Cuestionario=<2>;

6. Botón 'Enviar a gerencia': Se envía a Gerencia el cuestionario.
...
3. Mostrar tipps de respuesta: Selección del tipo de respuesta.
